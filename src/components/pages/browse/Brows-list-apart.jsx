...
const BrowsListApart = () => {
  ...
  const ListRoomsBystate = async () => {
    ...
    setLoading(true)
    await axios
      .get(`/api/v1/ListApartByState/${state}?pageNo=${pageNo}`)
      .then((response) => {
        setLoading(false)
        response.data.userData && setRoomList(response.data.userData);
  ...
  return (
    <>
      <Header />
      <section className="bg-light mb-80">
