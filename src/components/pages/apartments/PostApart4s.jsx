...
const PostApart4s = () => {
  ...
  const handleNext2 = async () => {
    ...
    await axios
      ...
      .then(() => {
        history.push("/process-apart-advert5");
      })
      .catch((err) => {
        setLoading2(false);
  ...
  const checkUserRegistered = async () => {
    await axios
      .get(`/api/v1/isUserRegistered/${PostFlatReducer.email}`)
      .then((response) => {
        console.log(response.data);
            ...
            <div className="property-info mt-30 bg-white py-40 px-30">
              <div className="text-center">
                <h5 className="color-primary">
                  Welcome {PostFlatReducer.email}, login to contnue
                </h5>
              </div>
